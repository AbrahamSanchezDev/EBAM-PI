...
const char* ssid = "Tenda_CB7FA0";
const char* password = "Holy55Killer";
const char* apiEndpoint = "http://192.168.2.195:3000/api/rfid"; // Reemplaza con tu IP local y puerto
...
void setup() {
  ...
  digitalWrite(BUZZER_PIN, LOW); // Asegura que el buzzer está apagado inicialmente
  
}

void beep(unsigned int duration) {
  digitalWrite(BUZZER_PIN, HIGH); // Enciende el buzzer
  delay(duration);                // Espera la duración del pitido
...
void registerScanToAPI(String uid) {
  if(WiFi.status() != WL_CONNECTED) {
    ...
    // Pitido de error de conexión (serie de pitidos cortos)
    for(int i=0; i<3; i++) {
      beep(100);
      delay(100);
    }
...
String httpRequestData = "{\"uid\":\"" + uid + "\",\"device_id\":\"" + nombreDispositivo + "\"}";
  ...
  if(httpResponseCode > 0) {
    ...
    // Opcional: pitido de confirmación de éxito
      beep(100);
      delay(100);
      beep(100);
